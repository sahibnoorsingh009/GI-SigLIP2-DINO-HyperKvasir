...
POLYP_FAMILY = ["polyps", "dyed-lifted-polyps", "dyed-resection-margins"]
...
def load_model(ckpt_path, model_name, n_classes, device):
    ...
    model = SigLIPClassifier(model_name, n_classes)
...
def main():
    ...
    ap.add_argument("--general_ckpt", required=True)
    ap.add_argument("--specialist_ckpt", required=True)
    ap.add_argument("--out_dir", required=True)
    ap.add_argument("--model_name", default="google/siglip2-base-patch16-224")
    ap.add_argument("--image_size", type=int, default=224)
    ap.add_argument("--batch_size", type=int, default=32)
    ap.add_argument("--num_workers", type=int, default=8)
    ap.add_argument("--route_topk", type=int, default=3)
    args = ap.parse_args()
    ...
    general, _ = load_model(args.general_ckpt, args.model_name, len(labels), device)

    spec_ckpt = torch.load(args.specialist_ckpt, map_location="cpu")
    spec_labels = spec_ckpt["labels"]
    spec_label_to_global = {i: label_to_id[l] for i, l in enumerate(spec_labels)}

    specialist = SigLIPClassifier(args.model_name, len(spec_labels))
    specialist.load_state_dict(spec_ckpt["model"], strict=True)
    specialist.to(device)
    specialist.eval()

    family_global_ids = set(label_to_id[l] for l in POLYP_FAMILY)

    y_true, y_pred, image_ids, routed_flags = [], [], [], []

    for x, y, ids in tqdm(loader, desc=f"routed {args.split}"):
        x = x.to(device, non_blocking=True)

        general_logits = general(x)
        topk = general_logits.topk(args.route_topk, dim=1).indices

        general_pred = general_logits.argmax(dim=1)

        spec_logits = specialist(x)
        spec_pred_local = spec_logits.argmax(dim=1)
        spec_pred_global = torch.tensor(
            [spec_label_to_global[int(i)] for i in spec_pred_local.cpu()],
            device=general_pred.device,
            dtype=general_pred.dtype,
        )

        final_pred = general_pred.clone()
        routed = []

        for i in range(x.shape[0]):
            topk_set = set(int(v) for v in topk[i].cpu().tolist())
            should_route = len(topk_set.intersection(family_global_ids)) > 0
            routed.append(should_route)
            if should_route:
                final_pred[i] = spec_pred_global[i]

        y_true.extend(y.numpy().tolist())
        y_pred.extend(final_pred.cpu().numpy().tolist())
        image_ids.extend(list(ids))
        routed_flags.extend(routed)

    metrics = compute_metrics(y_true, y_pred)
    metrics["routed_count"] = int(sum(routed_flags))
    metrics["total_count"] = int(len(routed_flags))
    metrics["routed_fraction"] = float(sum(routed_flags) / max(len(routed_flags), 1))
